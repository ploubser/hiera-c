            ...
            scope_t *scope, lookup_response_t *lookup_response)
{
  ...
  json_t *new_answer = json_null();

  if (strncmp(resolution_type, "array", 5) == 0)
    new_answer = json_array();      
  else
    new_answer = json_null();
  ...
  for (int i = 0; i <= json_array_size(hierarchy) - 1; i++) {
    ...
    if (strncmp(resolution_type, "array", 5) == 0) {
      json_array_append_new(sources, json_string(new_source));
      json_array_append_new(new_answer, answer);
    } 
    else {
      json_array_append_new(sources, json_string(new_source));
      new_answer = answer;
...
void lookup_key(const char *key, const char *source, json_t **answer)
{
  ...
  log_console(CONSOLE_LOG_DEBUG, "Searching for %s\n", source_file);

  json_error_t errors;
  json_t *data = json_load_file(source_file, 0, &errors); 
...
{
  log_console(CONSOLE_LOG_DEBUG, "Loading scope: %s\n", scope_file_path);
...
{
  log_console(CONSOLE_LOG_DEBUG, "Loading scope from string\n");
...
{
  log_console(CONSOLE_LOG_DEBUG, "Searching for Hiera config file: %s\n",
              config_file_path);

  if (file_exist(config_file_path) == 0)
    log_console(CONSOLE_LOG_DEBUG, "Found %s\n", config_file_path);
  else
    log_console(CONSOLE_LOG_ERR, "Cannot locate %s\n", config_file_path);

  json_error_t errors;
  json_t *data           = json_load_file(config_file_path, 0, &errors);
  config->hierarchy      = json_object_get(data, "hierarchy");
  config->data_directory = json_object_get(data, "datadir");

  json_incref(config->hierarchy);
  json_incref(config->data_directory);
  json_decref(data);
}
...
const char * parse_string(const char *string, const json_t *scope)
{
  ...
  const json_t *new_source = json_object_get(scope, submatch);
  free(submatch);
